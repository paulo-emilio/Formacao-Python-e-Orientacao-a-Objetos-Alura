...
def frequencia_letras_10_mais(texto):
    aparicoes_caracteres = Counter(texto.lower())
    total_caracteres = sum(aparicoes_caracteres.values())
    lista_texto = [(letra, frequencia / total_caracteres * 100) for letra, frequencia in aparicoes_caracteres.items()]
    dict_texto = Counter(dict(lista_texto))
    mais_comuns = dict_texto.most_common(10)
    for caractere, porcentagem in mais_comuns:
        print(f'|{caractere}| - {porcentagem:>5.2f}% ')


frequencia_letras_10_mais(texto1)
print('~~'*60)
frequencia_letras_10_mais(texto2)
